...
function translate(rawV) {
  const f = ((rawV - 16384) * 200) / 32768;
  ...
  for (let dev = 0; dev < 4; dev++) {
    ...
    if (result.length === 0) {
      ...
      for (let meas = 0; meas < results.length; meas++) {
        const fValue = result[meas].filteredValue === null ? "?" : translate(result[meas].filteredValue);
        const valStr = pad(5, fValue);
    ...
    switch (followupObject.command) {
      case "configuration":
        ...
        issFct(
          ...
          [
            iss.i2cMode.I2C_AD1,
            adsAddresses.setSlaveAddress(followupObject.deviceIndex, false),
  ...
  switch (step) {
    case "config-ads":
      if (resultAsArray[0] === 0) {
        results[followupObject.deviceIndex].forEach((channel) => {
          channel.filteredValue = null;
          printResults();
        });
        setTimeout(onIssDelayExpired, cfg.recurrenceInMs, followupObject);
      } else {
...
for (let i = 0; i < 4; i++) {
  ...
  if (!invalid) {
    issFifo.push(buildFollowupObject(i, 0, "configuration"));
  }
